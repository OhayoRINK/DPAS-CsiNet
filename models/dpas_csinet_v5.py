...
DPAS-CsiNet v5
==============
원본 DPAS 구조 기반, 두 가지만 추가:
1. Channel Attention (SE-style) — 인코더 feature 품질 향상
2. Decoder skip connection      — gradient flow 개선, 복원 안정화

학습 기법: CsiNet 원본과 동일 (Adam, MSE, lr=1e-3 고정)
파라미터 추가: ~300개 (Channel Attention SE block)
"""
...
# ── ★ 추가 1: Channel Attention (SE block) ───────────────────
class ChannelAttention(nn.Module):
    """
    Squeeze-and-Excitation style channel attention.
    AFG가 주파수 도메인 게이팅이라면,
    이건 공간 도메인 채널 중요도 재조정 → 상호보완.
    파라미터: 16→4→16 = 96개
    """
    def __init__(self, channels, reduction=4):
        super().__init__()
...
class DPASEncoder(nn.Module):
    def __init__(self, feedback_bits):
        ...
        # ★ Channel Attention: FC 직전 feature 품질 향상
    ...
    def forward(self, x):
        ...
        feat = self.ca(feat)             # ★ channel attention 적용
...
class DPASDecoder(nn.Module):
    ...
    def forward(self, codeword):
        ...
        x      = self.refine(x_init)
        # ★ skip connection: x_init을 직접 더해 gradient flow 개선
